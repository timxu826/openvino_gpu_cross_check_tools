...
def set_logger(lvl: str, file=None):
    ...
    if(file is not None):
        log.basicConfig(filename=file, level=lvl)
...
def build_parser():
    ...
    parser.add_argument('-bc', '--batch_check', type=int, default=0,
                        help='Increase output verbosity')
    parser.add_argument('-run', '--dry_run', action='store_true', default=False,
                        help='Increase output verbosity')
    modes.add_argument('--op_list', type=str, action=ExistingFileAction, help='Path to a file to load tensors from')
    return parser
...
def find_out_cct_mode(args):
    ...
    if args.dry_run:
        return 7
    # dump mode
    if args.batch_check==1:
        return 6
    if args.batch_check==2:
        return 5 
...
def read_multi_input_file(input_file: str, model_inputs: list, batch_size:int):
    ...
    for model_input in model_inputs:
        ...
        if model_input.any_name in npz:
            # just_tensor = npz[model_input.any_name].item(0)['tensor']
            just_tensor = npz[model_input.any_name]
            input_shape = list(just_tensor.shape)
            ...
            try:
                if(len(input_shape) == 4):
                    ...
                    just_tensor = np.reshape(just_tensor, input_shape)
            except:
            ...
            if batch_size == 2:
                if(input_shape[0] == batch_size):
                    reshaped_tensor = just_tensor
                elif(input_shape[0] == 1):
                    reshaped_tensor = np.concatenate((just_tensor, just_tensor), axis=0)
                elif(input_shape[0] > batch_size):
                    if(batch_size == 1):
                        reshaped_tensor = just_tensor[0,:]
                    else:
                        raise Exception(
                ...
                if(input_shape[0] == 1):
                    ...
                    if(batch_size == 1):
                        reshaped_tensor = just_tensor[0:1,:,:,:]
                    else:
                        raise Exception(
                            f'Can not find \'tensor\' parameter for input {model_input.any_name} in input file {input_file}')
            else:
                if(input_shape[0] == 1):
                    reshaped_tensor = just_tensor
                elif(input_shape[0] == 2):
                    reshaped_tensor = just_tensor[0:1,:,:,:]
                else:
                    raise Exception(
                            f'Not implemented Can not find \'tensor\' parameter for input {model_input.any_name} in input file {input_file}')
            if(reshaped_tensor is []):
                raise Exception(
                        f'Can not find \'tensor\' parameter for input {model_input.any_name} in input file {input_file}')
...
def read_torch_file(bin_file, model_input):
    log.info(f"Prepare torch file {str(bin_file)}")
    input_tensor = torch.load(bin_file)
    return input_tensor


def input_processing(model_path: str, model_inputs: list, input_file: str, batch_size: int):
    if input_file is None:
        ...
        for i in range(min(len(inputs), len(model_inputs))):
            ...
            if path.exists() and path.is_file():
                ...
                elif path.suffix.lower() in PYTORCH_EXTENSIONS:
                    input_data = read_torch_file(path, current_input)
                elif path.suffix.lower() in NUMPY_EXTENSIONS:
                    return read_multi_input_file(path, model_inputs, batch_size)
            elif path.is_dir():
...
def get_ops_list(all_ops: list, outputs: list, layers: str, op_list: str):
    if layers is not None and layers != 'None':
        if layers == 'all':
            return [op for op in all_ops if op.get_type_name() not in ['Constant', 'Result', 'Parameter']]
        elif layers == 'file':
            # Read the list back from the file using pickle
            import pickle
            with open(op_list, 'rb') as file:
                read_dict = pickle.load(file)
            read_list = [read_dict[item] for item in read_dict]
            output_ops = [op for op in all_ops if op.friendly_name in read_list]
            if len(output_ops) != len(read_list):
                log.warning("couldn't find some ops.")
            output_ops_filtered = [op for op in output_ops if op.get_type_name() not in ['Constant', 'Result', 'Parameter']]
            return output_ops
        else:
